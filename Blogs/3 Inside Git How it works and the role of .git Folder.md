...
- objects: All the changes are stored here with their first two hash code as a folder name for better storage. If u explore into this you will see files with commit hash. That file includes hash of previous commit which it is dependent on (parent).